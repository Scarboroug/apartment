...
<body>
							...
							<tr>
								<td>
									<span class="input-icon"> 
										<input autocomplete="off" id="nav-search-input" type="text" name="name" value="${pd.name}" placeholder="请输入关键词" /> 
									...
									<c:when test="${not empty varList}">
										<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr>
												...
												<%-- <td class='center' style="width: 150px;">${var.gender == '1' ? '男' : '女'}</td> --%>
												...
												<td style="width:200px;" class="center">
													<a href="javascript:;" onclick="quitHousing(${var.empId},${var.price });">退房</a> 
												</td>
											</tr>
										</c:forEach>
									</c:when>
	...
	<script type="text/javascript">
		...
		function quitHousing(empId, price)
		{
			/* var charge = 200 - price;	
			console.log(charge);
			if(charge > 0)
			{
				//alert("需退款" + charge + "元", {icon: 6});
				var message = "需退款" + charge + "元";
				parent.layer.alert(message, {icon: 1});
				//alert(message, {icon: 1});
//				layer.alert(message, {icon: 1});	
			}
			else if(charge < 0)
			{
				var message = "需支付" + charge + "元";
				parent.layer.alert(message, {icon: 1});
			}
			else
			{
